...
- In the Northwind database, what is the type of relationship between the
  ...
  In the Northwind database, the type of relationship between the Employee and Territory tables I can say is one to many. That may make more sense intuitively to me because there is a single employee usually gets his or her own territory to work on sales of product. In addtion to that, if someone looks at the employee-territory table, one employee can have multiple territorries and all the TerritoryIDs. I think those are uniques IDs which can imply on no two employees can have the same territory.
...
- What is a situation where a document store (like MongoDB) is appropriate, and
  ...
  Document stores are appropriate when I want to store separate documents inside of a collection rather than storing in columns and rows like SQL. I can say onne specific uses case could be a data structure that is like a resume. A JSON representation as opposed to the SQL schema might work better as this situation because I can query all of them with one query. A situation when a document database would not be appropriate is when a multitude of many to one relationships are presented and joins are needed for more insightful querrying.
...
The NewSQL is a new type of relational database, mostly built from scratch, that supports the relational data model and uses SQL as the primary interface. I think the NewSQL seeks to combine atomicity, cosistency, isolation, and durability of relatioal databases with the scalability of NoSQL. Also, it combines the reliability of SQL with the speed and performance of NoSQL.
